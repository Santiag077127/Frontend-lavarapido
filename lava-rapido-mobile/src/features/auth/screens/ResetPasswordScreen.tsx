      ...
      <KeyboardAvoidingView style={styles.keyboard} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          <View style={styles.content}>
            ...
            <Text style={[styles.subtitle, { color: theme.textSecondary }]}>Ingresa tu nueva contraseña para continuar.</Text>
            {passwordField('Nueva contraseña', password, setPassword, showPassword, () => setShowPassword(!showPassword), 'lock-closed-outline')}
            {passwordField('Confirmar contraseña', confirmPassword, setConfirmPassword, showConfirmPassword, () => setShowConfirmPassword(!showConfirmPassword), 'shield-checkmark-outline')}
            <TouchableOpacity style={[styles.button, { backgroundColor: theme.primary }]} onPress={handleReset} activeOpacity={0.8}><Text style={styles.buttonText}>Guardar contraseña</Text></TouchableOpacity>
          </View>
        </ScrollView>
